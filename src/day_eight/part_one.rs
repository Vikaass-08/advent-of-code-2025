...
pub fn solution() -> Result<i128, String> {
  ...
  for idx in 0..1000 {
    form_union(closest_pairs[idx].0, closest_pairs[idx].1, &mut parent, &mut size);
  }

  size.sort_by(|a, b| b.cmp(&a));

  dbg!(closest_pairs.len());
  dbg!(size[0]);
  dbg!(size[1]);
  dbg!(size[2]);

  let sum: i128 = size[0] * size[1] * size[2];

  Ok(sum)
}


fn form_union(d1: usize, d2: usize, parent: &mut Vec<usize>, size: &mut Vec<i128>) {
